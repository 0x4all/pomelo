...
var Service = function(app, opts) {
  ...
  this.app.event.on("handler_added",(record)=>{
    if(!record.serverType || !record.serviceName || !record.handler) {
      return;
    }
    var handlers = this.handlerMap[record.serverType];
    if(!handlers) {
      handlers = this.handlerMap[record.serverType] = {}
    }
    handlers[record.serviceName] = record.handler;
  })

  this.app.event.on("handler_removed",(record)=>{
    if(!record.serverType || !record.serviceName || !record.handler) {
      return;
    }
    var handlers = this.handlerMap[record.serverType];
    if(handlers) {
      handlers[record.serviceName] = undefined;
    }
  })
};
